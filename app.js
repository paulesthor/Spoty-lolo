document.addEventListener('DOMContentLoaded', async () => {
    ...
    let currentPlaylistSort = 'titre_asc';
    ...
    const sortPartitionsArray = (array, sortKey) => {
        let column = 'titre';
        let order = 'asc';
        if (sortKey === 'artiste_asc') { column = 'nom_artiste'; order = 'asc'; }
        else if (sortKey === 'annee_desc') { column = 'annee'; order = 'desc'; }
        else if (sortKey === 'date_ajout_desc') { column = 'date_ajout'; order = 'desc'; }
        ...
        array.sort((a, b) => {
            ...
            if (column === 'annee') { valA = Number(valA) || 0; valB = Number(valB) || 0; }
            else if (column === 'date_ajout') { valA = new Date(valA).getTime() || 0; valB = new Date(valB).getTime() || 0; }
    ...
    const loadPlaylistContent = async (pid, allPlaylistsData) => {
            ...
            <div class="playlist-actions-header">
                ...
                <div style="display:flex; align-items:center; gap:10px;">
                    <select id="pl-sort-select" class="btn" style="background-color: var(--card-color); border: 1px solid var(--highlight-color); color:var(--text-color); padding: 7px 15px; margin-right: 15px; border-radius: 6px; cursor: pointer;">
                        ...
                        <option value="date_ajout_desc" ${currentPlaylistSort === 'date_ajout_desc' ? 'selected' : ''}>Ajout (Récent)</option>
                    </select>
        ...
        const plSortSelect = document.getElementById('pl-sort-select');
        if (plSortSelect) {
            plSortSelect.addEventListener('change', (e) => {
                currentPlaylistSort = e.target.value;
                loadPlaylistContent(pid, allPlaylistsData);
            });
        }
